...
let contractSigningVoidApp = null;
...
frappe.pages["contract-signing-void"].on_page_load = function (wrapper) {
	ensureOrgUiCss();
	...
	mountEl.id = "contract-signing-void-arco-root";
	$main.append(mountEl);
	...
	contractSigningVoidApp = window.OrgUI.mountContractSigning(mountEl, { status: "void" });
};

frappe.pages["contract-signing-void"].on_page_leave = function () {
	try {
		contractSigningVoidApp?.unmount?.();
	} catch (e) {
		console.warn("[contract-signing-void] unmount failed", e);
	}
	contractSigningVoidApp = null;
};
