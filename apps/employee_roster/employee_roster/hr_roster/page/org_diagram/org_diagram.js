...
// License: MIT
// Loaded from hr_roster/page/org_diagram/org_diagram.js — do not duplicate in hooks.page_js.

(function () {
	const PAGE = "org-diagram";
...
.org-diagram-loading,.org-diagram-error{padding:24px;color:#667085;text-align:center}
.org-diagram-error{color:#b42318}
`;
	...
	function ensure_org_diagram_stylesheet() {
	...
	function load_diagram($main) {
		frappe.call({
			...
			callback(r) {
				if (!r.message) {
					$main.find(".orgchart-diagram").html(
						`<div class="org-diagram-error">${__("未获取到架构图数据")}</div>`
					);
					return;
				}
			...
			error(err) {
				const msg = (err && (err.message || err.exc)) || __("未知错误");
				$main.find(".orgchart-diagram").html(
					`<div class="org-diagram-error">${__("加载架构图失败")}：${esc(msg)}</div>`
				);
			},
		});
	}
	...
	frappe.pages[PAGE].on_page_load = function (wrapper) {
		...
		$main.html(`
			...
			<div class="orgchart-diagram"><div class="org-diagram-loading">${__("加载中...")}</div></div>
		`);
