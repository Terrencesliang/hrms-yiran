...
let contractSigningPendingApp = null;
...
frappe.pages["contract-signing-pending"].on_page_load = function (wrapper) {
	ensureOrgUiCss();
	...
	mountEl.id = "contract-signing-pending-arco-root";
	$main.append(mountEl);
	...
	contractSigningPendingApp = window.OrgUI.mountContractSigning(mountEl, { status: "pending" });
};

frappe.pages["contract-signing-pending"].on_page_leave = function () {
	try {
		contractSigningPendingApp?.unmount?.();
	} catch (e) {
		console.warn("[contract-signing-pending] unmount failed", e);
	}
	contractSigningPendingApp = null;
};
